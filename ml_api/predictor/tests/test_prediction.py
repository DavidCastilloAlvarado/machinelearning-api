...
class Prediction(TestCase):
    ...
    def test_prediction(
        ...
        response = client.post(
            "/api/ml/model/register/", INPUT_MODEL_DATA, format="json"
        )
        ...
        # create prediction
        response = client.post(
            f"/api/ml/model/heart-failure/predict/?id={id_model}",
            self.x_input,
            format="json",
        )
        assert response.status_code == 200

    def test_multi_prediction(
        self,
    ):
        """
        ...
        # create prediction
        response = client.post(
            f"/api/ml/model/heart-failure/predict/?id={id_model}&many=true",
            [self.x_input, self.x_input, self.x_input],
            format="json",
        )
        assert response.status_code == 200
