...
class PerformPredictionHeartFailureSerializer(serializers.Serializer):
    ...
    chestPainType = serializers.CharField(
        help_text="""chest pain type [TA: Typical Angina, ATA: Atypical Angina,
    ...
    fastingBS = serializers.FloatField(
        help_text="fasting blood sugar [1: if FastingBS > 120 mg/dl, 0: otherwise]"
    )
    restingECG = serializers.CharField(
        help_text="""resting electrocardiogram results [Normal: Normal, ST: having ST-T wave
    ...
    maxHR = serializers.FloatField(
        help_text="maximum heart rate achieved [Numeric value between 60 and 202]"
    )
    exerciseAngina = serializers.CharField(
        help_text="exercise-induced angina [Y: Yes, N: No]"
    )
    oldpeak = serializers.FloatField(
        help_text="ST [Numeric value measured in depression]"
    )
    sTSlope = serializers.CharField(
        help_text="""the slope of the peak exercise ST segment [Up: upsloping, Flat:
